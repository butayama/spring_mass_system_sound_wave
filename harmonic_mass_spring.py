...
"""Please, run the program on Python 3, not on Python 2, in Python 2 sometimes gives some problems with the RK4 
method, I don't know where. Maybe the line t_steps = int(t_f/delta_t), but who knows. It send the masses to position 
e265 sometimes, and Python 3 with the same variables made it right. """
    ...
    for jt in range(t_steps - 1):
        ...
        for n in range(1, n_balls):
            if n <= (n_balls - 2):
                ...
                kv[1, n] = delta_t * k_spr_m * f_n_in(x[n, jt] + 0.5 * kx[0, n], x[n + 1, jt] + 0.5 * kx[0, n + 1],
                                                      x[n - 1, jt] + 0.5 * kx[0, n - 1])
            elif n == (n_balls - 1):
                kx[1, n] = delta_t * (v[n, jt] + kv[0, n])
        ...
        for n in range(1, n_balls):
            if n <= (n_balls - 2):
                ...
                kv[2, n] = delta_t * k_spr_m * f_n_in(x[n, jt] + 0.5 * kx[1, n], x[n + 1, jt] + 0.5 * kx[1, n + 1],
                                                      x[n - 1, jt] + 0.5 * kx[1, n - 1])
            elif n == (n_balls - 1):
                kx[2, n] = delta_t * (v[n, jt] + kv[1, n])
        ...
        for n in range(1, n_balls):
            if n <= (n_balls - 2):
                ...
                kv[3, n] = delta_t * k_spr_m * f_n_in(x[n, jt] + kx[2, n], x[n + 1, jt] + 0.5 * kx[2, n + 1],
                                                      x[n - 1, jt] + 0.5 * kx[2, n - 1])
            elif n == (n_balls - 1):
                kx[3, n] = delta_t * (v[n, jt] + kv[2, n])
